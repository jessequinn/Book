...
\long\def\CF@ifnextchar#1#2#3{%
	\let\CF@ifnextchar@tok=#1% <- espace ind�sirable, bugfix dans 1.31
...
\def\CF@if@firsttokmatch#1#2{% est ce que #1 et #2 commencent par les m�mes tokens ?
...
\def\CF@valuetomacro#1[#2]#3{% #1= macro  #2=valeur d�faut  #3=cl�
...
\def\resetchemfig{%
	\setKVdefault[chemfig]{%
		atom style          ={},% code tikz mis � la fin de every node/.style
		chemfig style       ={},% code tikz mis � la fin de l'arugment optionnel de \tikzpicture
		bond join           =false,
		fixed length        =false,
		...
		arrow double coeff  =0.6,%  [nouveau] coefficient de la petite fleche par rapport � la grande
...
\def\chemskipalign{%
	\CF@doifempty\CF@bond@outcontentsaved% sauf si un \chemskipalign a �t� fait � l'atome pr�c�dent
		{\global\let\CF@bond@outcontentsaved\CF@bond@outcontent}% sauvegarder l'atome d'o� vient la liaison
...
\def\def@submol@a#1#2#3{% #1 nom, #2 = code si liaison arrive de droite , #3 = code si liaison arrive de gauche
	\endgroup
	\ifcat\relax\expandafter\noexpand\CF@firsttonil#1\@nil% si #1 est une s�quence de contr�le
...
\def\CF@seek@node#1#2#3{% cherche un noeud au d�but de #1 l'assigne dans la sc #2 et met le reste dans #3
	\let#2\empty
	\def#3{#1}%
...
% #2 re�oit le type de liaisons (1 pour -, 2 pour =, 3 pour ~)
\def\CF@bond@findcode#1#2{%
	...
	\edef#2{%
		...
		\ifx~\CF@toks@a3\else
		\ifx>\CF@toks@a4\else
		\ifx<\CF@toks@a5\else0% si 0 --> il y a une erreur non due � l'utilisateur
		\fi\fi\fi\fi\fi}%
	\ifnum#2>\thr@@% si c'est une liaison de Cram
		\CF@exp@after{\futurelet\CF@toks@a\CF@gobtonil}{\CF@gobarg#1\@nil}% chope le caract�re suivant
		\CF@ifx:\CF@toks@a
			{\edef#2{\number\numexpr#2+2}}% si c 'est un ":", signe du pointill�, ajoute 2
			{\CF@ifx|\CF@toks@a{\edef#2{\number\numexpr#2+4}}{}}% si c 'est un "|", signe du triangle �vid�, ajouter 4
	\fi
}
...
\def\CF@analyse@bond#1#2{%
	...
	\ifnum#2=\tw@% si c'est une double liaison, regarde s'il y a un + ou - derri�re
	...
	\else
		\ifnum#2>5 % si c'est une laision de Cram pointill�e ou triangle �vid�
			\CF@exp@second{\def\CF@remain@afterbond}{\CF@gobtwoargs#1}% mange un caract�re de plus
		\fi
	\fi
	...
	\ifCF@incycle
		...
		\let\CF@current@length\default@length% et on ignore la longueur de liaison sp�cifi�e
	\fi
...
% Argument limit�s l�gitimes ici car #2 (qui est ce qui suit "@{<nom>}" dans l'argument optionnel) ne DOIT
...
\def\CF@seek@submol{%
	...
	\CF@exp@macroarg\CF@if@firsttokmatch{\CF@remain@molecule}!%
		{\CF@eexp@second{\def\CF@remain@molecule}{\expandafter\CF@gobarg\CF@remain@molecule}% enl�ve le "!"
...
\def\CF@insert@emptygroup#1{% ins�re {} au d�but de la sc #1
...
\def\CF@chemfig@d#1#2#3#4{%
	...
	\begingroup
		...
		\CF@valuetomacro\CF@atom@sep[3em]{atom sep}% espacement des atomes li�s
		...
		\let\CF@last@action\z@% 0=d�but du dessin 1=trac� d'un noeud 2=trac� d'une liaison
		...
		\def\default@angle{0}%
		\def\default@stringangle{:0}% angle pris par d�faut si le champ est vide
		\def\default@length{1}%
		\let\default@fromatom\empty% numero de l'atome d'o� partent les liaisons par d�faut
		\let\default@toatom\empty% num�ro de l'atome o� arrivent les laisons par d�faut
...
\def\CF@chemfig@e#1{% #1 est le code de la mol�cule
	\ifnum\CF@last@action=\thr@@
		\ifCF@incycle
			\def\default@angle{0}%
		\else
			\ifnum\CF@cnt@cycle=\z@% si c'est le d�but d'un cycle
				\pgfmathsetmacro\default@angle{\CF@previous@angle+180+\CF@cycle@anglecorrection}% on met la liaison � +180� + correction
			\else
				\pgfmathsetmacro\default@angle{\CF@previous@angle-90+180/\CF@cycle@num}% sinon � la bissectrice du sommet du cycle
			\fi
		\fi
	...
	\ifCF@incycle% si on commence un cycle
		...
		\ifCF@cyclearc% on doit tracer l'arc de cercle dans le cycle ?
			...
			\node[at=(arccenter),shift=(\CF@cyclearc@startangle:\CF@cyclearc@radius pt)](initarc){};% le d�but de l'arc
...
\def\CF@chemfig@f{%
	\let\CF@next@action\CF@chemfig@f% � priori, on reboucle
	\CF@ifx\CF@remain@molecule\empty
		...
		\CF@ifx\empty\CF@current@atomgroup% pas de noeud pour commencer ?
			...
			\CF@if@firsttokin@a{-=<>~}% la suite est une liaison
				{\ifnum\CF@last@action=\tw@% c'est la deuxi�me liaison cons�cutive ?
					\CF@insert@emptygroup\CF@remain@molecule% ins�re un groupe vide
				...
				% s'il ne reste plus rien apr�s la liaison (sans tenir compte de l'espace devant)-> ins�re un groupe vide
					\CF@insert@emptygroup\CF@remain@molecule
				\fi
				}%
				{\edef\CF@bond@outnode{\CF@bond@outnode}% �value le l'atome de d�part de liaison
				\CF@ifx(\CF@toks@a% une parenth�se pour commencer ?
					{\ifnum\CF@last@action=\tw@% il y avait une liaison juste avant ?
						\CF@insert@emptygroup\CF@remain@molecule
					\else
						\CF@exp@macroarg\CF@grab@submol{\CF@remain@molecule}%
						\begingroup
							\ifCF@incycle\let\CF@last@action\thr@@\fi% on �tait dans un cycle
							\CF@incyclefalse
					...
					{\CF@ifx\CF@remain@molecule\empty
						{\let\CF@next@action\endgroup}
						{% ce qui reste apr�s le noeud courant n'est pas vide, ne commence pas par "-=~", ni par une parenth�se
						\CF@ifx*\CF@toks@a% un cycle ?
							...
							\else
								...
								\CF@eexp@macroarg\CF@if@firsttokmatch{\expandafter\CF@gobarg\CF@remain@molecule}*%
									{\CF@eexp@second{\def\CF@remain@molecule}{\expandafter\CF@gobarg\CF@remain@molecule}% enl�ve la 1er �toile
...
% Cr�� 4 noeuds au dessus et au dessous des noeuds #1 et #2
% � une distance de #3 du noeud #1 et #4 du noeud #2
...
\def\CF@draw@bond#1#2#3#4#5{% #1=type de liaison #2 et #3:nom de noeuds de d�but et fin #4 et #5: contenu des atomes de d�but et fin
	...
	\or% 2 = liaison double
		...
		\or
			...
			\begingroup% ajuste �ventuellement les longueurs des l(0,0iaisons doubles
				\ifCF@incycle
		...
		\or
			\CF@create@normnodes{#2@@}{#3@@}\CF@double@sep\CF@double@sep
			\CF@draw@axisbond{#2}{#3}% trace la liaison simple dans l'axe\CF@draw@bond@a(#2@@)--(#3@@);
			\begingroup% ajuste �ventuellement les longueurs des liaisons doubles
				\ifCF@incycle
					\ifdim\CF@start@offset=\z@
	...
	\or% 5 = liaison Cram creuse de #3 vers #2
		...
		\let\CF@joinbond\z@
	\or% 6 = liaison Cram pointill�e de #2 vers #3
		\scope
			\CF@create@normnodes{#2@@}{#3@@}{\CF@cram@basewidth/2}{}%
		...
		\endscope
		\let\CF@joinbond\z@
	\or% 7 = liaison Cram pointill�e de #3 vers #2
		\scope
			\CF@create@normnodes{#3@@}{#2@@}{\CF@cram@basewidth/2}{}%
		...
		\endscope
		\let\CF@joinbond\z@
	\or% 8 = liaison cram rectangle �vid� de #2 vers #3
		\CF@create@normnodes{#2@@}{#3@@}{\CF@cram@basewidth/2}{}%
		\expandafter\draw\expandafter[\CF@current@bondstyle,line join=bevel](#2@@1)--(#2@@2)--(#3@@)--cycle;
		\let\CF@joinbond\z@
	\or% 9 = liaison cram rectangle �vid� de #3 vers #1
		\CF@create@normnodes{#3@@}{#2@@}{\CF@cram@basewidth/2}{}%
...
\def\CF@draw@axisbond@join#1#2{% dessine une liaison simple dans l'axe avec raccord r�trograde
	...
	\else
		\CF@ifx\CF@previous@tikz\CF@current@tikz
			...
			\CF@ifx\CF@previous@atomgroup\CF@empty@node
				{\CF@ifx\CF@previous@bondangle\empty% si d�but mol�cule
					{\CF@draw@bond@a(#1@@)--(#2@@);
					}
					{\ifdim\CF@start@offset=\z@
						\ifCF@incycle
...
\def\CF@hook@drawall{% dessine tous les crochets contenus dans la sc \CF@hook@draw
	\CF@doifnotempty\CF@hook@drawlist
		{\expandafter\CF@hook@drawfirst\CF@hook@drawlist\@nil% trace un lien de crochet � crochet
		\CF@hook@drawall
		}%
}
...
\def\CF@draw@atomgroup#1#2#3{% #1=angle d'arriv�e de la liaison #2=numero atome sur lequel arrive la liaison #3=groupe d'atomes
	\expandafter\let\expandafter\CF@bond@outcontent% assigne le contenu de l'atome d'o� part la liaison
		\csname
	...
	\CF@remove@movearg\CF@current@atomgroup% enl�ve les "@{<nom>}"
	\CF@if@instr{#3}?%
	...
	\edef\CF@opt@string{anchor=\ifnum\CF@last@action=\z@ base\else\ifCF@incycle center\else\ifCF@macrofixedbondlength 180+#1\else center\fi\fi\fi,at=(CF@node),\CF@node@style}% premier atome de la mol�cule affich�
	\loop
	...
	\ifnum\CF@last@action=\tw@% s'il faut tracer une liaison
		\gdef\CF@cycle@anglecorrection{0}% alors c'est qu'un cycle ne peut pas commencer la mol�cule : annulation de la correction d'angle
		\CF@draw@bond\CF@bond@type{\CF@bond@outnode}{n\number\CF@cnt@groupnumber-\number\CF@cnt@atomnumber}\CF@previous@atomgroup\CF@current@atomgroup
	\fi
	\let\CF@last@action\@ne% met la derni�re action � 1 : affichage d'un noeud
	\loop
...
% enl�ve tous les "@{nom}" de la sc #1
...
% enl�ve le premier "@{<nom>}" de l'argument et l'assigne � #2
				...
				{\CF@ifx\@sptoken\CF@toks@a
					...
					{\CF@ifx\bgroup\CF@toks@a
						...
						{\CF@exp@macroarg\CF@ifcar@isupperletter{\CF@firsttonil#1\@nil}%
							...
							{\CF@ifx?\CF@toks@a
								{\CF@exp@macroarg\CF@if@firsttokmatch{\CF@gobarg#1}[% un crochet apr�s le "?"
								...
								\CF@exp@twomacroargs\CF@if@instr{\CF@hook@list}{\expandafter(\CF@hook@currentname)}% crochet d�j� d�fini ?
									{\CF@exp@macroarg\CF@hook@parselist{\CF@hook@currentname}% chercher les caract�ristiques du crochet sauvegard�
									...
									\CF@eadd@tocs\CF@hook@drawlist{\expandafter{\CF@current@atom}}% ajoute les 4 arguments � la liste des crochets � tracer
...
\def\CF@hook@parselist#1{% #1 est le nom du noeud � retrouver
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%           S C H � M A S      R � A C T I O N N E L S           %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\def\CF@schemestart@a[#1]{%
	...
	\CF@ifx\empty\CF@arrow@style
		...
		{\CF@exp@second{\CF@preadd@tocs\CF@arrow@style}{\CF@default@arrow@style,}}% se rajoute apr�s le style par d�faut
...
\def\CF@schemestart@b[#1][#2]{%
	\ifnum\CF@scheme@nest=\@ne% la commande n'est pas imbriqu�e ?
		\CF@compound@cnt\z@
	\fi
...
\def\CF@arrow@b(#1.#2[#3]--#4.#5[#6])#7{%
	\def\CF@current@arrowtype{#7}% nom de la fl�che
...
% #7 : angle fl�che    #8 : longueur fl�che    #9 : style tikz de la fl�che
\def\CF@arrow@e(#1.#2[#3]--#4.#5[#6])#7#8#9{%
	...
	\else
		...
		\fi
	\fi
	\CF@arrow@gobspaces% mange les espaces puis ex�cute \CF@next@action
}
...
\def\CF@arrow@f#1#2#3#4{% #1=angle   #2=longueur    #3=ancre d�part   #4=ancre arriv�e
...
% trace un fl�che initi�e par (@nom--@nom)
% #1=type de fl�che   #2=nom depart   #3=ancre d�part    #4=nom arriv�e    #5=ancre arriv�e    #6=style fl�che
...
\def\CF@merge@b#1--(#2)[#3]{%
	...
	\CF@merge@c#1(\relax)% trace les lignes entre les noeuds pr�c�dents et la ligne de jonction
...
% pose des noeuds d�cal�s de la dimension #1 � (\CF@arrow@start@node) et (\CF@arrow@end@node)
...
----------------------------------------------------------------------
v1.0f      2012/02/24
    - correction d'un bug avec \definesubmol, les catcodes n'�taient
...
v1.1       2015/02/13
    ...
    - la jonction entre deux liaisons cons�cutives dans l'axe peut
      �tre activ� avec \enablebondjoin et d�sactiv� avec
